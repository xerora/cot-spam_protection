...
<!-- BEGIN: HAS_SPAM -->
	...
	<table class="table table-striped table-condensed sp-table" style="margin-bottom: 0px;">
		<thead>
			<tr>
			<th style="width: 3%;">{SP_CHECKALL}</th> 
			<th style="width: 17%;">{PHP.L.sp_username}</th>
			<th style="width: 40%;">{PHP.L.sp_content_summary} ({PHP.L.sp_without_markup})</th>
			<th style="width: 14%;">&nbsp; &nbsp; {PHP.L.Date} &nbsp; {SP_ITEM_ORDER_DATE_ASC_ICON} {SP_ITEM_ORDER_DATE_DESC_ICON}</th>
			<th style="width: 12%;">{PHP.L.sp_actions}</th>
			</tr>
		</thead>
		<tbody>
	<!-- BEGIN: SPAM_ITEMS -->
		...
		<!-- BEGIN: VIEW_ITEM -->
			<tr id="item_{SP_ITEM_VIEW_ID}" class="view_item">
				...
				<table style="border-top: 2px solid #c9c9c9; border-bottom: 1px solid #ccc; width: 100%;" class="sp-table">
					<!-- BEGIN: ITEM_DATA -->
						<tr style="border-bottom: 1px solid #ddd; width: 100%;">
							...
							<td colspan="4" style="width: 80%; border: 0px; border-right: 2px solid #c9c9c9; background-color: #fff; overflow: auto;">{SP_ITEM_VIEW_DATA_VALUE}</td>
						</tr>
					<!-- END: ITEM_DATA -->
